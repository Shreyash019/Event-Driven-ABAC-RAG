...
export class AuthService {
  ...
  async refresh(presentedRefreshToken: string): Promise<AuthResult> {
    ...
    if (result.status === 'reuse_detected') {
      ...
      this.logger.warn(
        `refresh token reuse detected; revoked family=${result.familyId} user=${result.userId}`,
      );
    ...
    const access = await this.tokens.signAccessToken(this.toIdentity(user));
    return {
  ...
  private toIdentity(user: User): AbacIdentity {
    return {
      sub: user.id,
      tenant: user.tenant,
